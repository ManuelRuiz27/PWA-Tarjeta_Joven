...
export default function NotificationsToggle() {
  ...
  useEffect(() => {
    if (!supported) return;
    getSubscription().then((s) => setEnabled(Boolean(s))).catch(() => {});
  }, [supported]);
  ...
  return (
    <div>
      <h2><FormattedMessage id="notifications.title" defaultMessage="Notificaciones Push" /></h2>
      <p aria-live="polite">Estado: {statusLabel}</p>
      <button onClick={handleToggle} disabled={!supported || loading || permission === 'denied'}>
        {loading ? '...' : enabled ? <FormattedMessage id="notifications.toggle.disable" defaultMessage="Desactivar" /> : <FormattedMessage id="notifications.toggle.enable" defaultMessage="Activar" />}
      </button>
